...
namespace INSN.Web.ApiRest.Controllers.Home
{
    ...
    public class DocumentoLegalController : ControllerBase
    {
        ...
        {
            var response = await _service.ListAsync(request.Documento, request.Descripcion,request.Area, request.TipoDocumentoId, request.Estado, request.EstadoRegistro, request.Page, request.Rows);
